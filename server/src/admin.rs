...
pub async fn login(
    ...
    let host = host::Entity::find().filter(host::Column::Domain.eq(input.domain.trim()))
    .one(db)
    .await?;

    if host.as_ref().map(|host|get_session_login(&session).map(|login|login.hosts.contains(&host.id)).is_ok()).unwrap_or_default() {
        // already logged in
...
async fn fetch_host_txt(instance_url: &str, client: &Client, config: &Config) -> LoginResult<String> {
    let mut request_url = Url::parse(&instance_url).map_err(|_|LoginError::InstanceUrl(instance_url.to_string()))?;
    request_url.set_path(&[".",&config.login_token_name].concat());
    request_url.set_query(None);
    let result = client.get(request_url.clone()).send().await.map_err(|e|LoginError::HttpFailure(request_url.clone(), e))?;
    match true { // result.status().is_success()
        true => Ok(result.text().await.map_err(|e: reqwest::Error|LoginError::InvalidResponse(request_url,e.to_string()))?),
        false => Err(LoginError::ServerResponse(result.status().as_u16(),result.text().await.unwrap_or_default())),
    }
}
...
pub async fn history_json(
    ...
    let history: Vec<health_check::Model> = health_check::Entity::find().filter(health_check::Column::Host.eq(host.id))
    .order_by_asc(health_check::Column::Time).limit(20).all(db).await?;
...
pub async fn errors_view(
    ...
    let errors = check_errors::Entity::find().filter(check_errors::Column::Host.eq(host.id))
    .order_by_desc(check_errors::Column::Time).limit(20).all(db).await?;
...
async fn get_all_login_hosts(session: &Session, db: &DatabaseConnection) -> Result<Vec<host::Model>> {
    let login = get_session_login(&session)?;

    let host_res = host::Entity::find().filter(host::Column::Id.is_in(login.hosts.iter().map(|v|*v)))
    .all(db).await?;
    Ok(host_res)
}

/// Get wanted [host::Model] for current [Session] if valid for this user
async fn get_specific_login_host(wanted_host_id: i32, session: &Session, db: &DatabaseConnection) -> Result<host::Model> {
    let login = get_session_login(&session)?;
    ...
    let host_res = host::Entity::find().filter(host::Column::Id.eq(wanted_host_id)).one(db).await?;
    match host_res {
        None => {
            session.delete();
